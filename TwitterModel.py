...
class Account1:
    ...
    def SearchTweetsFromPerson(self, person, count=1):

        payload = {'screen_name': person,
                   'count': count
                   }

        with self.session.get('https://api.twitter.com/1.1/statuses/user_timeline.json', params=payload, timeout=10) as response:
            # print(response.text)
    ...
    def SearchTweets(self, theme = None):
        ...
        if theme == None:
            ...
            with self.session.get('https://api.twitter.com/1.1/search/tweets.json', params=payload, timeout=5) as response:
                # print(response.text)
                return response.json()
            ...
            with self.session.get('https://api.twitter.com/1.1/search/tweets.json', params=payload,
                                  timeout=10) as response:
                # print(response.text)
                return response.json()
    ...
    def GetMessages(self):
        ...
        with self.session.get('https://api.twitter.com/1.1/direct_messages/events/list.json?count=50') as response:

            lastMessages = response.json()['events']

            for message in lastMessages:
                try:
                    if message['message_create']['source_app_id'] == '3033300':
                        continue
                    else:

                        # print(self.Get_User_Username(message['message_create']['sender_id']), '-', message['message_create']['message_data']['text'])
                        self.AddToDB(self.Get_User_Username(message['message_create']['sender_id'], message['message_create']['message_data']['text']))

                except:

                    # print(self.Get_User_Username(message['message_create']['sender_id']), '-',
                    #       message['message_create']['message_data']['text'])
                    self.AddToDB(self.Get_User_Username(message['message_create']['sender_id'],
                                                        message['message_create']['message_data']['text']))

    def AddToDB(self, from_, text):

        conn = sqlite3.connect('DataBases/Main_Database.db', check_same_thread=False)
        cur = conn.cursor()

        all_used_msgs = cur.execute(f'''SELECT * FROM used_messages''')
        all_used_msgs = [[i[2], i[3]] for i in all_used_msgs]

        if [f'{self.auth_token}:{self.name}', text] not in all_used_msgs:
            cur.execute(f'''INSERT INTO messages VALUES (?,?,?,?)''', (str(uuid.uuid4()), from_, f'{self.auth_token}:{self.name}', text))
            conn.commit()

            cur.execute(f'''INSERT INTO used_messages VALUES (?,?,?,?)''',
                        (str(uuid.uuid4()), from_, f'{self.auth_token}:{self.name}', text))
            conn.commit()
    ...
    def GetTweets(self, screen_name):
        ...
        with self.session.get(f'https://api.twitter.com/1.1/users/lookup.json?screen_name={screen_name}') as response:
            # pprint(response.json())
            # print('\n\n')
            # print(response.json()[0]['status']['text'])

            return response.json()

    def sendMessage(self):

        payload = {"event": {"type": "message_create", "message_create": {"target": {"recipient_id": "1348911885505028100"}, "message_data": {"text": "Hello Kun. I won MEME code"}}}}

        with self.session.post(f'https://api.twitter.com/1.1/direct_messages/events/new.json', json=payload) as response:
            # pprint(response.json())
            # print('\n\n')
            # print(response.json()[0]['status']['text'])
    ...
    def GetLastPost(self, user_id):
        ...
        with self.session.get('https://api.twitter.com/1.1/statuses/user_timeline.json', params=payload,
                              timeout=10) as response:
            # print(response.text)
            return response.json()
